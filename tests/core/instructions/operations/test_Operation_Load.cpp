...
TEST_CASE("Invalid width throws", "[Instructions::Operations::Load]") {
  ...
  for (const auto& opcode : { Opcode::LD, Opcode::LDZX }) {
    CHECK_THROWS(
      (load.setData({ opcode, InstructionData::Width { 0b11 }, 0, sources }), load.execute(state))
    );
  }
}
...
TEST_CASE("Invalid addressing mode throws", "[Instructions::Operations::Load]") {
  ...
  for (const auto& opcode : { Opcode::LD, Opcode::LDZX }) {
    const auto genLoad = [&] {
      using enum InstructionData::Width;
      return Instruction {
        InstructionData { opcode, Word, 0, sources }
      };
    };
...
TEST_CASE("Valid load correctly loads", "[Instructions::Operations::Load]") {
  ProcessorState state {};
  state.memory.set<uint32_t>(FIRST_VALID_ADDRESS, 0xDEAD'BEEF);
  ...
  Instruction load {
    { Opcode::LD, InstructionData::Width::Word, 1, sources }
  };
  load.execute(state);
  ...
  load.execute(state);
  CHECK(state.registers[2].get<uint32_t>() == 0xDEAD'BEEF);
  CHECK(!state.registers.getFLAGS().getBit(Register::FlagIndex::Zero));
  CHECK(state.registers.getFLAGS().getBit(Register::FlagIndex::Sign));
...
TEST_CASE("Valid sign/zero extension", "[Instructions::Operations::Load]") {
  ...
  Instruction load {
    { Opcode::LD, InstructionData::Width::Byte, 1, sources }
  };
  load.execute(state);
  CHECK(state.registers[1].get<uint32_t>() == 0xFFFF'FFEF);

  load.setData({ Opcode::LD, InstructionData::Width::HalfWord, 1, sources });
  load.execute(state);
  CHECK(state.registers[1].get<uint32_t>() == 0xFFFF'BEEF);
