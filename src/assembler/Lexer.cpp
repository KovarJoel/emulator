...
namespace assembler {
  std::ostream& operator<<(std::ostream& out, const Lexer::Token& token) {
    ...
    switch (token.type) {
      ...
      case Lexer::TokenType::RightParen:    type = "RightParen";    break;
      case Lexer::TokenType::LeftSquare:    type = "LeftSquare";    break;
      case Lexer::TokenType::RightSquare:   type = "RightSquare";   break;
      case Lexer::TokenType::NewLine:       type = "NewLine";       break;
      case Lexer::TokenType::EndOfFile:     type = "EndOfFile";     break;
      case Lexer::TokenType::String:        type = "String";        break;
      case Lexer::TokenType::Number:        type = "Number";        break;
      case Lexer::TokenType::Identifier:    type = "Identifier";    break;
    }
  ...
  void Lexer::runOnLine(size_t line_index) {
    ...
    for (size_t column_index = 0; column_index < line.size(); ++column_index) {
      if (line[column_index] == ':') {
        m_tokens.emplace_back(TokenType::Colon, line.substr(column_index, 1), line_index, column_index);
      }
      else if (line[column_index] == ',') {
        m_tokens.emplace_back(TokenType::Comma, line.substr(column_index, 1), line_index, column_index);
      }
      else if (line[column_index] == '.') {
        ...
        m_tokens.emplace_back(TokenType::Equal, line.substr(column_index, 1), line_index, column_index);
      }
      else if (line[column_index] == '(') {
        m_tokens.emplace_back(TokenType::LeftParen, line.substr(column_index, 1), line_index, column_index);
      }
      else if (line[column_index] == ')') {
        m_tokens.emplace_back(TokenType::RightParen, line.substr(column_index, 1), line_index, column_index);
      }
      else if (line[column_index] == '[') {
        m_tokens.emplace_back(TokenType::LeftSquare, line.substr(column_index, 1), line_index, column_index);
      }
      else if (line[column_index] == ']') {
        m_tokens.emplace_back(TokenType::RightSquare, line.substr(column_index, 1), line_index, column_index);
      }
      else if (line[column_index] == '"') {
