...
namespace assembler {
  ...
  bool Parser::isMnemonic(const Lexer::Token& token) {
    if (token.type != Lexer::TokenType::Identifier) return false;
  ...
  void Parser::parseString() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::String) {
      throw createError(ErrorType::Parser_ExpectedString, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
  ...
  void Parser::parseNumber() {
    ...
    if (m_remaining_lexer_tokens[0].type == Lexer::TokenType::Number) {
      ...
      throw createError(ErrorType::Parser_ExpectedNumber, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }

    if (is_signed) {
      assert(m_remaining_lexer_tokens.size() >= 2);
      if (m_remaining_lexer_tokens[1].type != Lexer::TokenType::Number) {
        throw createError(ErrorType::Parser_ExpectedNumberAfterMinus, m_remaining_lexer_tokens[1].line, m_remaining_lexer_tokens[1].column);
      }
    }
    ...
    if (number_token.value.size() >= 2 && number_token.value[0] == '0' && isBaseSymbol(number_token.value[1])) {
    ...
    if (is_signed) {
      int32_t value{};
      conversion_result = std::from_chars(string_value.data(), string_value.data() + string_value.size(), value, base);
      result = static_cast<uint32_t>(value);
    }
    else {
      conversion_result = std::from_chars(string_value.data(), string_value.data() + string_value.size(), result, base);
    }
    
    if (conversion_result.ec == std::errc::result_out_of_range) {
      throw createError(ErrorType::Parser_NumericValueOutOfRange, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    
    assert(conversion_result.ec == std::errc{} && conversion_result.ptr == string_value.data() + string_value.size());
    
    m_tokens.emplace_back(Immediate{ result }, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
  ...
  void Parser::parseNewLine() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::NewLine) {
      throw createError(ErrorType::Parser_ExpectedEndOfLine, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    m_tokens.emplace_back(NewLine{}, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(1);
  }
  ...
  void Parser::parseOperandSeparator() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Comma) {
      throw createError(ErrorType::Parser_ExpectedOperandSeparator, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(1);
  }
  ...
  void Parser::parseRegisterOperand() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Identifier) {
      throw createError(ErrorType::Parser_ExpectedRegisterOperand, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    ...
    try {
      ...
      throw createError(ErrorType::Parser_InvalidRegisterAddress, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    ...
    m_tokens.emplace_back(Register{ static_cast<uint32_t>(register_address) }, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(1);
  }
  ...
  void Parser::parseJumpOperandsLine() {
    ...
    if (m_remaining_lexer_tokens[0].type == Lexer::TokenType::Number) {
      ...
      throw createError(ErrorType::Parser_ExpectedJumpTarget, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
  ...
  void Parser::parseBranchOperandsLine() {
    ...
    if (m_remaining_lexer_tokens[0].type == Lexer::TokenType::Number) {
      ...
      throw createError(ErrorType::Parser_ExpectedBranchTarget, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
  ...
  void Parser::parseOperandsLine(core::instructions::Opcode opcode) {
    ...
    for (size_t i = 0; i < src_count; ++i) {
      ...
      if (m_remaining_lexer_tokens[0].type == Lexer::TokenType::Minus || m_remaining_lexer_tokens[0].type == Lexer::TokenType::Number
        ...
        throw createError(ErrorType::Parser_ExpectedOperand, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
      }
  ...
  const std::vector<Parser::Token>& Parser::run(std::span<const Lexer::Token> lexer_tokens) {
    ...
    while (true) {
      ...
      // - instructions                   Identifier, optional Dot+Identifier, Identifier/Minus/Number/NewLine
      // data:
  ...
  void Parser::parseInstructionLine() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Identifier) {
      throw createError(ErrorType::Parser_ExpectedMnemonic, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    if (!isMnemonic(m_remaining_lexer_tokens[0])) {
      throw createError(ErrorType::Parser_UnknownMnemonic, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }

    using namespace core::instructions;
    const auto opcode = opcodeFromString(m_remaining_lexer_tokens[0].value);
    m_tokens.emplace_back(Mnemonic{ opcode }, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(1);

    assert(!m_remaining_lexer_tokens.empty());
    if (opcodeGetType(opcode) == OpcodeType::Memory && m_remaining_lexer_tokens[0].type == Lexer::TokenType::Dot) {
      parseWidthSpecifier();
    }
  ...
  void Parser::parseInitializerLine() {
    ...
    const auto& last_used = old_lexer_tokens[old_lexer_tokens.size() - m_remaining_lexer_tokens.size()];

    if (!valid_end) {
      throw createError(ErrorType::Parser_ExpectedInitializerValue, last_used.line, last_used.column);
    }
  ...
  void Parser::parseWidthSpecifier() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Dot) {
      throw createError(ErrorType::Parser_ExpectedWidthSpecifier, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    assert(m_remaining_lexer_tokens.size() >= 2);

    if (m_remaining_lexer_tokens[1].type != Lexer::TokenType::Identifier) {
      throw createError(ErrorType::Parser_ExpectedWidthSpecifier, m_remaining_lexer_tokens[2].line, m_remaining_lexer_tokens[2].column);
    }

    if (lower(m_remaining_lexer_tokens[1].value) == "b") {
      m_tokens.emplace_back(WidthSpecifier{ WidthSpecifier::Width::Byte }, m_remaining_lexer_tokens[1].line, m_remaining_lexer_tokens[1].column);
      m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(2);
    }
    else if (lower(m_remaining_lexer_tokens[1].value) == "h") {
      m_tokens.emplace_back(WidthSpecifier{ WidthSpecifier::Width::HalfWord }, m_remaining_lexer_tokens[1].line, m_remaining_lexer_tokens[1].column);
      m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(2);
    }
    else if (lower(m_remaining_lexer_tokens[1].value) == "w") {
      m_tokens.emplace_back(WidthSpecifier{ WidthSpecifier::Width::Word }, m_remaining_lexer_tokens[1].line, m_remaining_lexer_tokens[1].column);
      m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(2);
    }
    else {
      throw createError(ErrorType::Parser_InvalidWidthSpecifier, m_remaining_lexer_tokens[1].line, m_remaining_lexer_tokens[1].column);
    }
  }
  ...
  void Parser::parseVariableLine() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Equal) {
      throw createError(ErrorType::Parser_ExpectedEquals, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
    m_remaining_lexer_tokens = m_remaining_lexer_tokens.subspan(1);
  ...
  void Parser::parseBranch() {
    ...
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Identifier) {
      throw createError(ErrorType::Parser_ExpectedBranchLabel, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }
  ...
  void Parser::parseFunctionLine() {
    assert(!m_remaining_lexer_tokens.empty());
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Identifier || lower(m_remaining_lexer_tokens[0].value) != FUNCTION) {
      throw createError(ErrorType::Parser_ExpectedFunctionDeclaration, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }

    assert(m_remaining_lexer_tokens.size() >= 2);
    if (m_remaining_lexer_tokens[1].type != Lexer::TokenType::Identifier) {
      throw createError(ErrorType::Parser_MissingFunctionName, m_remaining_lexer_tokens[1].line, m_remaining_lexer_tokens[1].column);
    }

    assert(m_remaining_lexer_tokens.size() >= 3);
    if (m_remaining_lexer_tokens[2].type != Lexer::TokenType::Colon) {
      throw createError(ErrorType::Parser_MissingFunctionColon, m_remaining_lexer_tokens[2].line, m_remaining_lexer_tokens[2].column);
    }

    m_tokens.emplace_back(FunctionLabel{ m_remaining_lexer_tokens[1].value }, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
  ...
  void Parser::parseSegmentLine() {
    assert(!m_remaining_lexer_tokens.empty());
    if (m_remaining_lexer_tokens[0].type != Lexer::TokenType::Identifier || lower(m_remaining_lexer_tokens[0].value) != SEGMENT) {
      throw createError(ErrorType::Parser_ExpectedSegment, m_remaining_lexer_tokens[0].line, m_remaining_lexer_tokens[0].column);
    }

    assert(m_remaining_lexer_tokens.size() >= 2);
    if (m_remaining_lexer_tokens[1].type != Lexer::TokenType::Colon) {
      throw createError(ErrorType::Parser_MissingSegmentColon, m_remaining_lexer_tokens[1].line, m_remaining_lexer_tokens[1].column);
    }
    ...
    {
      throw createError(ErrorType::Parser_MissingSegmentType, m_remaining_lexer_tokens[2].line, m_remaining_lexer_tokens[2].column);
    }
    
    m_tokens.emplace_back(
      Segment{ lower(m_remaining_lexer_tokens[2].value) == SEGMENT_CODE ? Segment::Type::Code : Segment::Type::Data },
