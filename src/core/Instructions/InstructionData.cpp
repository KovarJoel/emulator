...
namespace core::instructions {
  ...
  void InstructionData::decode(std::span<const std::byte> data) {
    ...
    if (opcodeGetType(opcode) == OpcodeType::Branch) {
      ...
      *this = InstructionData{ opcode, Width::Word, 0, { RegisterOrImmediate{ SourceMode::Immediate, jump_target }}};
      return;
    }

    const uint16_t remaining_data = (static_cast<uint16_t>(data[1]) << 8) | static_cast<uint8_t>(data[2]);
    ...
    for (size_t i = 0; i < sources.size(); ++i) {
      const auto register_address = decodeField.template operator()<uint32_t>(ENCODING_WIDTH_REGISTER_ADDRESS);
      ...
      const auto immediate = decodeImmediate(data.subspan(ENCODING_SIZE_MIN + immediates_count * ENCODING_SIZE_IMMEDIATE));
